...
void terminal_process_string(char *str) {
  ...
  if (strcmp(argv[0], "ping") == 0) {
    ...
    if (argc == 2) {
      ...
      if (duty > 0.0 && duty < 0.9) {
        ...
        float curr, curr2, ind1, ind2;
        mcpwm_foc_measure_inductance(duty, 200, &curr, &curr2, &ind1, &ind2);
  ...
  else if (strcmp(argv[0], "display_capacity") == 0) {
    commands_printf("Capacity Estimate: %f percent",(double) (100.0*mc_interface_get_capacity()));
  }
  /*else if (strcmp(argv[0], "estimate_pack_res") == 0) {
    if (argc == 3) {
      float current = -1.0;
      float erpm = -1.0;
      sscanf(argv[1], "%f", &current);
      sscanf(argv[2], "%f", &erpm);
      commands_printf("beginning test",(double) current,(double) erpm);
      float volt_start =  mc_interface_get_voltage();
      float volt_end = 0;
      float current_end = 0;
      if (current >= 0.0 && erpm >= 0.0) {
        commands_printf("Setting Current: %f Motor Amps ",(double) current);
        mcpwm_foc_set_openloop(current, erpm);
        mcpwm_foc_set_openloop2(current, erpm);
        ...
        for (int i = 0; i < 10; i++) {
          mcpwm_foc_set_openloop(current, erpm);
          mcpwm_foc_set_openloop2(current, erpm);
          volt_end +=  mc_interface_get_voltage();
          current_end += mc_interface_get_tot_current_in();
          chThdSleepMilliseconds(20);
        }
        volt_end = volt_end/10.0;
        current_end = current_end/10.0;
        commands_printf("Measured: %f Volts, %f Amps ",(double)(volt_start - volt_end), (double)current_end);
        mc_interface_set_current(0.0,0.0);
        float res_measured = (volt_start - volt_end)/current_end;
        commands_printf("Resistance Measured: %f ohms",(double) res_measured);
      }
      else {
        commands_printf("Invalid argument(s).\n");
      }
  ...
  else if (strcmp(argv[0], "estimate_pack_capacity") == 0) {
    if (argc == 3) {
      float current = -1.0;
      float erpm = -1.0;
      sscanf(argv[1], "%f", &current);
      sscanf(argv[2], "%f", &erpm);
      commands_printf("beginning test",(double) current,(double) erpm);
      float ah_start = mc_interface_get_amp_hours(0);
      float ah_diff = 0;
      float capacity_start =  mc_interface_get_capacity();
      ...
      if (current >= 0.0 && erpm >= 0.0) {
        ...
        mcpwm_foc_set_openloop(current, erpm);
        mcpwm_foc_set_openloop2(current, erpm);
        chThdSleepMilliseconds(400000);
        mc_interface_set_current(0.0,0.0);
        chThdSleepMilliseconds(20000);

        capacity_diff = capacity_start - mc_interface_get_capacity();
        ah_diff =  mc_interface_get_amp_hours(0) - ah_start;
        commands_printf("Measured: %f mAh Consumed, %f Battery Percent Consumed, %f mAh Capacity Estimate",(double) ah_diff, (double)capacity_diff, (double) (ah_diff/capacity_diff));
      }
      else {
        commands_printf("Invalid argument(s).\n");
      }
