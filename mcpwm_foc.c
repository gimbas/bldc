...
typedef struct {
  ...
  float avg_current_tot2;
  float avg_voltage_tot2;
...
static volatile temperature_values m_temp_values;
static volatile int temp_indice = 0;
...
static volatile int m_curr3_sum;
...
static volatile int m_curr3_offset;
...
static volatile bool measure;
static volatile bool motor_one_control = 0;
...
static void observer_update(float v_alpha, float v_beta, float i_alpha, float i_beta, float dt, volatile float *x1, volatile float *x2, volatile float *phase);
...
static void svm(float alpha, float beta, uint32_t PWMHalfPeriod, uint32_t* tAout, uint32_t* tBout, uint32_t* tCout, uint32_t *svm_sector);
...
#define TIMER_UPDATE_DUTY_2(duty1, duty2, duty3) \
    TIM8->CR1 |= TIM_CR1_UDIS; \
    TIM8->CCR1 = duty1; \
    TIM8->CCR2 = duty3; \
    TIM8->CCR3 = duty2; \
...
void mcpwm_foc_init(volatile mc_configuration *configuration) {
  ...
  dmaStreamAllocate(STM32_DMA_STREAM(STM32_DMA_STREAM_ID(2, 4)), 3, (stm32_dmaisr_t)mcpwm_foc_adc_int_handler, (void *)0);
...
void mcpwm_foc_set_configuration(volatile mc_configuration *configuration) {
  m_conf = configuration;
  //mcpwm_foc_set_configuration2(configuration);
  m_control_mode = CONTROL_MODE_NONE;
  m_state = MC_STATE_OFF;
  stop_pwm_hw();
...
void mcpwm_foc_encoder_detect(float current, bool print, float *offset, float *ratio, bool *inverted) {
  ...
  for (int i = 0; i < it_rat; i++) {
    ...
    for (float i = phase_ovr_tmp; i < phase_ovr_tmp + (2.0 / 3.0) * M_PI; i += (2.0 * M_PI) / 500.0) {
      m_phase_now_override = i;
      chThdSleepMilliseconds(1);
    }
...
void mcpwm_foc_measure_resistance(float current, int samples, float *res, float *res2) {
  ...
  m_control_mode = CONTROL_MODE_CURRENT;
  m_state = MC_STATE_RUNNING;
  mcpwm_foc_start_resistance2(current);
  // Disable timeout
  systime_t tout = timeout_get_timeout_msec();
  float tout_c = timeout_get_brake_current();
  timeout_reset();
  ...
  m_samples.avg_voltage_tot2 = 0.0;
  ...
  const float current_avg2 = m_samples.avg_current_tot2 / (float)m_samples.sample_num;
  const float voltage_avg2 = m_samples.avg_voltage_tot2 / (float)m_samples.sample_num;
  // Stop
  m_id_set = 0.0;
  m_iq_set = 0.0;
  m_phase_override = false;
  ...
  m_state = MC_STATE_OFF;
  stop_pwm_hw();
  mcpwm_foc_stop_resistance2();

  // Enable timeout
  timeout_configure(tout, tout_c);

  mc_interface_unlock();
  *res = (voltage_avg / current_avg) * (2.0 / 3.0);
  *res2 = (voltage_avg2 / current_avg2) * (2.0 / 3.0);
}
...
void mcpwm_foc_measure_inductance(float duty, int samples, float *curr, float *curr2, float *l1, float *l2) {
  ...
  float t = (float)TIM1->ARR * m_samples.measure_inductance_duty / (float)SYSTEM_CORE_CLOCK - (float)(MCPWM_FOC_INDUCTANCE_SAMPLE_CNT_OFFSET + MCPWM_FOC_INDUCTANCE_SAMPLE_RISE_COMP) / (float)SYSTEM_CORE_CLOCK;

  float avg_current2 = m_samples.avg_current_tot2 / (float)m_samples.sample_num;
  float avg_voltage2 = m_samples.avg_voltage_tot2 / (float)m_samples.sample_num;
...
bool mcpwm_foc_one_turn_openloop(float current, bool dir) {
  ...
  mcpwm_foc_hall_detect_start2(current);
  chThdSleepMilliseconds(150);
  if(dir){
    for (int i = 0; i < 180; i++) {
      m_phase_now_override = (float)i * M_PI / 90.0;
      mcpwm_foc_set_phase_override2((float)i * M_PI / 90.0);
      chThdSleepMilliseconds(1);
    }
  } else{
    for (int i = 180; i >= 0; i--) {
...
bool mcpwm_foc_hall_detect(float current, uint8_t *hall_table) {
  ...
  int hall_iterations2[8];

  int hall_read1[8];
  int hall_read2[8];
  int hall_states_registered1 = 0;
  int hall_states_registered2 = 0;

  memset(sin_hall, 0, sizeof(sin_hall));
  memset(cos_hall, 0, sizeof(cos_hall));
  memset(hall_iterations, 0, sizeof(hall_iterations));
  memset(sin_hall2, 0, sizeof(sin_hall2));
  memset(cos_hall2, 0, sizeof(cos_hall2));
  memset(hall_iterations2, 0, sizeof(hall_iterations2));
  memset(hall_read1, 0, sizeof(hall_read1));
  memset(hall_read2, 0, sizeof(hall_read2));
  // Forwards
  for (int i = 0; i < 360; i++) {
    m_phase_now_override = (float)i * M_PI / 180.0;
    chThdSleepMilliseconds(1);
    int hall = read_hall();
    int hall2 = read_hall2();
    hall_read1[hall]++;
    hall_read2[hall2]++;
  }
  for (int i = 0; i < 8; i++) {
    if (hall_read1[i] > 5) {
      hall_states_registered1++;
    }
    if (hall_read2[i] > 5) {
      hall_states_registered2++;
    }
  }
  bool swapped;
  if(hall_states_registered2 > 5 && hall_states_registered1<6){
    swapped = 1;
  }
  ...
  for (int j = 0; j < 4; j++) {
    for (int i = 0; i < 360; i++) {
      m_phase_now_override = (float)i * M_PI / 180.0;
      mcpwm_foc_set_phase_override2((float)i * M_PI / 180.0);
      chThdSleepMilliseconds(1);
  ...
  for (int i = 0; i < 8; i++) {
    if (hall_iterations2[i] > 30) {
      float ang = atan2f(sin_hall2[i], cos_hall2[i]) * 180.0 / M_PI;
      utils_norm_angle(&ang);
      hall_table[i + 8] = (uint8_t)(ang * 200.0 / 360.0);
    }
    else {
      hall_table[i + 8] = 255;
      fails++;
...
void mcpwm_foc_adc_int_handler(void *p, uint32_t flags) {
  ...
  /*
   * TODO: Change this to a switch statement based on an enum:
   * MOT1_V0
   * MOT1_V7
   * MOT2_V0
   * MOT2_V7
   *
   * This loop runs at 2*PWM_FREQ and each time only one routine for one motor needs to be called
   * Right now it is kind of a confusing bunch of if statements but works well.
   */
  ...
  if(temp_count>5){
    temp_count = 0;
    switch (temp_indice) {
    case SELECT_MOSFET_TEMP_1:
      m_temp_values.mosfet_temp_1 = NTC_TEMP(ADC_IND_TEMP_MOS);
      ENABLE_MOT_TEMP1();
      temp_indice = SELECT_MOTOR_TEMP_1;
      break;
    case SELECT_MOTOR_TEMP_1:
      m_temp_values.motor_temp_1 = NTC_TEMP_MOTOR(m_conf->m_ntc_motor_beta);
      ENABLE_MOS_TEMP2();
      temp_indice = SELECT_MOSFET_TEMP_2;
      break;
    case SELECT_MOSFET_TEMP_2:
      m_temp_values.mosfet_temp_2 = NTC_TEMP(ADC_IND_TEMP_MOS);
      ENABLE_MOT_TEMP2();
      temp_indice = SELECT_MOTOR_TEMP_2;
      break;
    case SELECT_MOTOR_TEMP_2:
      m_temp_values.motor_temp_2 = NTC_TEMP_MOTOR(m_conf->m_ntc_motor_beta);
      ENABLE_MOS_TEMP1();
      temp_indice = SELECT_MOSFET_TEMP_1;
      break;
    }
  }
  ...
  bool is_v7_1 = !(TIM1->CR1 & TIM_CR1_DIR);
  bool is_v7_2 = !(TIM8->CR1 & TIM_CR1_DIR);
  motor_one_control = (is_v7_1 == is_v7_2);

  WWDG_SetCounter(100);
  if (!m_samples.measure_inductance_now) {
    if(motor_one_control){
      if (is_v7_1) {
        return;
      }
    }else{
      if(is_v7_2){
        ...
        m_curr2_sum += ADC_Value[ADC_IND_CURR3];
        m_curr3_sum += ADC_Value[ADC_IND_CURR4];
        mcpwm_foc_handler2();
        return;
      }
    }
  ...
  if (m_samples.measure_inductance_now) {
    ...
    if (!motor_one_control && is_v7_2) {
      int curr2 = ADC_Value[ADC_IND_CURR3];
      int curr3 = ADC_Value[ADC_IND_CURR4];
      curr2 -= m_curr2_offset;
      curr3 -= m_curr3_offset;
      if (inductance_state == 0) {
        ...
        TIMER_UPDATE_DUTY_2(duty_cnt, 0, duty_cnt);
      }
      else if (inductance_state == 3) {
        m_samples.avg_current_tot2 += -((float)curr3 * FAC_CURRENT);
        m_samples.avg_voltage_tot2 += GET_INPUT_VOLTAGE();
        TIMER_UPDATE_DUTY_2(0, 0, 0);
      }
      else if (inductance_state == 5) {
        TIMER_UPDATE_DUTY_2(0, duty_cnt, duty_cnt);
      }
      else if (inductance_state == 6) {
        m_samples.avg_current_tot2 += -((float)curr2 * FAC_CURRENT);
        m_samples.avg_voltage_tot2 += GET_INPUT_VOLTAGE();
        TIMER_UPDATE_DUTY_2(0, 0, 0);
      }
      else if (inductance_state == 8) {
        TIMER_UPDATE_DUTY_2(0, 0, duty_cnt);
      }
      else if (inductance_state == 9) {
        m_samples.avg_current_tot2 += -((float)curr2 * FAC_CURRENT + (float)curr3 * FAC_CURRENT);
        m_samples.avg_voltage_tot2 += GET_INPUT_VOLTAGE();
      ...
      return;
    }else if (motor_one_control && is_v7_1) {
      if (inductance_state == 0) {
  ...
  if (m_state == MC_STATE_RUNNING) {
    ...
    if (m_control_mode == CONTROL_MODE_SPEED && fabsf(m_speed_pid_set_rpm) < m_conf->s_pid_min_erpm) {
      control_duty = true;
      duty_set = 0.0;
    }
    ...
    if (control_duty) {
      ...
      if (fabsf(duty_set) < (duty_abs - 0.05) || (SIGN(m_motor_state.vq) * m_motor_state.iq) < m_conf->lo_current_min) {
    ...
    case FOC_SENSOR_MODE_SENSORLESS:
      ...
      if (!m_phase_override) {
        if (duty_abs < m_conf->foc_sl_d_current_duty) {
          id_set_tmp = utils_map(duty_abs, 0.0, m_conf->foc_sl_d_current_duty, fabsf(m_motor_state.iq_target) * m_conf->foc_sl_d_current_factor, 0.0);
        }
        else {
          id_set_tmp = 0.0;
        }
    ...
    utils_saturate_vector_2d(&id_set_tmp, &iq_set_tmp, utils_max_abs(m_conf->lo_current_max, m_conf->lo_current_min));
  ...
  m_motor_state.duty_now = SIGN(m_motor_state.vq) * sqrtf(m_motor_state.mod_d * m_motor_state.mod_d + m_motor_state.mod_q * m_motor_state.mod_q) / SQRT3_BY_2;
...
static THD_FUNCTION(timer_thread, arg) {
  ...
  for (;;) {
    ...
    float openloop_rpm = utils_map(fabsf(m_motor_state.iq_target), 0.0, m_conf->l_current_max, 0.0, m_conf->foc_openloop_rpm);
    ...
    if (m_state == MC_STATE_RUNNING) {
      ...
      const volatile float vd_tmp2 = mcpwm_foc_get_vd2();
      const volatile float vq_tmp2 = mcpwm_foc_get_vq2();
      const volatile float id_tmp2 = mcpwm_foc_get_id2();
      const volatile float iq_tmp2 = mcpwm_foc_get_iq2();
      m_samples.avg_current_tot += sqrtf(SQ(id_tmp) + SQ(iq_tmp));
      m_samples.avg_voltage_tot += sqrtf(SQ(vd_tmp) + SQ(vq_tmp));
      m_samples.avg_current_tot2 += sqrtf(SQ(id_tmp2) + SQ(iq_tmp2));
      m_samples.avg_voltage_tot2 += sqrtf(SQ(vd_tmp2) + SQ(vq_tmp2));
      m_samples.sample_num++;
    }

    // Update and the observer gain.
    m_gamma_now = utils_map(fabsf(m_motor_state.duty_now), 0.0, 1.0, m_conf->foc_observer_gain * m_conf->foc_observer_gain_slow, m_conf->foc_observer_gain);
...
static void do_dc_cal(void) {
  ...
  m_curr3_offset = m_curr3_sum / m_curr_samples;
#ifdef HW_HAS_3_SHUNTS
  m_curr2_offset = m_curr2_sum / m_curr_samples;
#endif
  mcpwm_set_curr_offset(m_curr2_offset, m_curr3_offset);
  DCCAL_OFF();
  m_dccal_done = true;
}
...
static void control_current(volatile motor_state_t *state_m, float dt) {
  ...
  // Saturation
  utils_saturate_vector_2d((float*)&state_m->vd, (float*)&state_m->vq, (2.0 / 3.0) * max_duty * SQRT3_BY_2 * state_m->v_bus);
...
static int read_hall(void) {
  ...
  return utils_middle_of_3_int(h1_1, h1_2, h1_3) | (utils_middle_of_3_int(h2_1, h2_2, h2_3) << 1) | (utils_middle_of_3_int(h3_1, h3_2, h3_3) << 2);
}
