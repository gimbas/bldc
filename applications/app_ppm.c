  ...
  for(;;) {
    ...
    case PPM_CTRL_TYPE_DUTY_NOREV:
      ...
      if (!(pulses_without_power < MIN_PULSES_WITHOUT_POWER && config.safe_start)) {
        mc_interface_set_duty(utils_map(servo_val, -1.0, 1.0, -mcconf->l_max_duty, mcconf->l_max_duty),utils_map(servo_val, -1.0, 1.0, -mcconf->l_max_duty, mcconf->l_max_duty));
        send_current = true;
      }
      break;
    ...
    case PPM_CTRL_TYPE_PID_NOREV:
      ...
      if (!(pulses_without_power < MIN_PULSES_WITHOUT_POWER && config.safe_start)) {
        mc_interface_set_pid_speed(servo_val * config.pid_max_erpm,servo_val * config.pid_max_erpm);
        send_current = true;
      }
      break;
    ...
    if (pulses_without_power < MIN_PULSES_WITHOUT_POWER && config.safe_start) {
      ...
      mc_interface_set_brake_current(timeout_get_brake_current(),timeout_get_brake_current());
      continue;
    }
